...
public class MapTransformationGestureDetector {
    private static final String TAG = MapTransformationGestureDetector.class.getSimpleName();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                float x = event.getX(event.findPointerIndex(ptrID1));
                float y = event.getY(event.findPointerIndex(ptrID1));
                if (mListener != null) {
